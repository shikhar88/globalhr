...
function apply_none(file,type_file,external){
	//console.log('asd');
    //apply_img(file,type_file,external);
    if ($('#popup').val()==1) var window_parent=window.opener; else var window_parent=window.parent;
    //console.log($(window_parent.document.getElementById( $('#track').val())));
    var path = $('#cur_dir').val();
    var base_url = $('#base_url').val();
    var ext=file.split('.').pop();
    var fill='';
    ...
    var editor=top.tinymce.activeEditor;
    editor.dom.remove(editor.dom.select("[data-type='watermark']"));
          var img=editor.dom.create("img",{scr:fill,"data-type":'watermark','data-file':file});
          editor.dom.add(editor.getBody(),img);
    
    top.showToast('info','Watermark image changed. Please click the Save button to apply the changes.');
